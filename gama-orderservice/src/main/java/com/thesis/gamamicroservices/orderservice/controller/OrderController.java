...
public class OrderController {
    ...
    private static final String GET_TOTAL_PRICE_LOG = "Order: {} price was requested for payment";
    ...
    @ResponseStatus(HttpStatus.OK)
    public Double getTotalOfOrder(@PathVariable int orderId) throws NoDataFoundException, AlreadyExistsException {
        logger.info(GET_TOTAL_PRICE_LOG, orderId);
        return this.orderService.getTotalOfOrder(orderId);
    }

    @PutMapping(path="/paymentconfirmed/{orderId}")
    @ResponseStatus(HttpStatus.OK)
    public void paymentConfirmed(@PathVariable int orderId) throws NoDataFoundException {
        logger.info(GET_TOTAL_PRICE_LOG, orderId);
