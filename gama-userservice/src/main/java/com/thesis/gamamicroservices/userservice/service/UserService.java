...
public class UserService {
    ...
    @Autowired
    public UserService(UserRepository userRepository, AddressRepository addressRepository, JwtTokenUtil jwtTokenUtil, PasswordEncoder bcryptEncoder, RestTemplate restTemplate) {
 ...
 **/
    ...
    public void deleteUser(String authorizationToken) throws NoDataFoundException {
        ...
        if(userRepository.existsById(userId)) {
            ...
            try {
                restTemplate.delete(PRODUCT_DELETE_REVIEW_URL + userId);
            } catch (HttpStatusCodeException ex) {
                System.out.println(ex.getStatusCode().toString());
                System.out.println("Review Service is currently down. Try again later");
            } catch (Exception e) {
                e.printStackTrace();
                System.out.println("Review Service is currently down. Try again later");
            }
